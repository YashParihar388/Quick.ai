...
const BlogTitles = () => {
  ...
  const blogCategories =['General', 'Technology', 'Business', 'Health', 
    ...
    const submitHandler = async(e)=>{
      ...
      try {
        setLoading(true); 
        const prompt=`Generate 5 catchy and SEO-friendly blog titles for a blog post about "${input}" in the category of ${selectedCategory}.
        The titles should be engaging and relevant to the topic.`;
  ...
  text-slate-700'>
    ...
    border-gray-200'>
        ...
        <input onChange={(e)=>setInput(e.target.value)} value={input} type='text'   className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border
         ...
         to-[#8E37EB] text-white px-4 py-2 mt-6 text-sm rounded-1g cursor-pointer'>
          
          {
            // CORRECT: Spaces added and border color defined
          loading?<span className='w-4 h-4 my-1 rounded-full border-2
           border-white border-t-transparent animate-spin'></span>:<Hash className='w-5'/>
          }
          Generate title
         </button>
     </form>
